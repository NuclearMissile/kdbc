...
abstract class Table(private val name: String? = null) : ColumnOrTable {
    ...
    protected inline fun <reified T : Any> column(
        name: String,
        ddl: String? = null,
        noinline getter: (ResultSet.(String) -> T)? = null
    ): Column<T> {
    ...
    override fun toString() =
        if (tableAlias.isNullOrBlank() || tableAlias == tableName) tableName else "$tableName $tableAlias"
